...
class Engine {
  ...
  async runByScript(
    ...
    if (this.options.safari && this.options.safari.useSimulator) {
      const getDevices = 'xcrun simctl list devices -j';
      try {
        const { stdout } = await execa.command(getDevices, {
          shell: true,
          timeout: 3000
        });
        ...
        for (let result of totalResult) {
          result.info.ios = {
            deviceName: iosSimulatorDeviceName + ' simulator',
        ...
        log.error(
          'Could not get device name from simualator using %s',
          getDevices
        );
      }
    }
