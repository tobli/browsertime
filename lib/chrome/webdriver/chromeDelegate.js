...
class ChromeDelegate {
  ...
  async beforeEachURL(runner, url) {
    if (this.options.cookie && this.options.cookieDomain) {
      await this.cdpClient.setCookies(
        this.options.cookieDomain,
        this.options.cookie
      );
    } else if (this.options.cookie && url) {
  ...
  async onCollect(runner, index, result) {
    ...
    if (
      ...
      for (let longTask of result.browserScripts.pageinfo.longTask) {
        ...
        if (firstContentfulPaint && longTask.startTime > firstContentfulPaint) {
          totalBlockingTime +=
            longTask.duration - (this.options.minLongTaskLength || 50);
        }
    ...
    if (this.android) {
      har.log._android = await this.android.getModel();
    }
